...
const FILE_ROWS: [FileFieldConfig, FileFieldConfig][] = [
  [
...
export interface IndividualSearchData {
  dob: string;
  email: string;
  nin: string;
  phone: string;
...
interface IndividualClientStepProps {
  onSearch: (data: IndividualSearchData) => void;
  onCancel: () => void;
}

export function IndividualClientStep({
  onSearch,
  onCancel,
}: IndividualClientStepProps) {
  ...
  return (
    <>
      ...
      <Box sx={{ mt: 2.5 }}>
        <Typography
          sx={{ fontSize: 14, fontWeight: 700, color: "#2A2A2A", mb: 1.5 }}
        >
          Check for Existing Client
        </Typography>

        <Typography
          sx={{ fontSize: 11, fontWeight: 700, color: "#2A2A2A", mb: 0.25 }}
        >
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 0.9 }}>
          {FILE_ROWS.map(([left, right], i) => (
            <Box
              key={i}
              sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0.8 }}
            >
              <FileUploadField config={left} />
              <FileUploadField config={right} />
            </Box>
          ))}
        </Box>
        ...
        >
          <BrandButton
            ...
            size="sm"
            onClick={onCancel}
            className="!text-[#f97316] !font-normal !px-4 !py-[6px] !px-[14px] hover:!bg-[#FFF0E9]"
          >
            Cancel
          </BrandButton>
          <BrandButton
