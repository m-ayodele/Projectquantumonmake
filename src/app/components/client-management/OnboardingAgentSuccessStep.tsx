...
interface OnboardingAgentSuccessStepProps {
  record?: AgentRecord;
  onBackToHome?: () => void;
  onViewClientDetails?: () => void;
}

export function OnboardingAgentSuccessStep({
  record = {
    clientId: "PO/7657/09",
  ...
  onBackToHome,
  onViewClientDetails,
}: OnboardingAgentSuccessStepProps) {
  return (
    <Box className="flex flex-col h-full">
      ...
      <Box className="mb-6 flex-shrink-0">
        ...
        <Typography sx={{ fontSize: 13, color: "#6B7280" }}>
          Here are the details of the agent as newly registered
        </Typography>
      </Box>
        ...
        <Box className="grid grid-cols-3 gap-4">
          <Box>
            ...
            <StatusPill
              ...
              color={
                record.verificationStatus === "Verified" ? "green" : "orange"
              }
            />
          </Box>
          <Box>
      ...
      {/* Next actions — only 2 cards as per image */}
      <Box
        sx={{
